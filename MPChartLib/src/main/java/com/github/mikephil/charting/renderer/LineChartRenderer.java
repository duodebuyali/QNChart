...
//                        (cur.getY() - curDy) * phaseY, cur.getX(), cur.getY() * phaseY);
                /**
                 * 这里绘制点之前的贝塞尔曲线，采取2点控制的方法
                 * 每段线段的中点平移到端点处与x轴平行
                 * */
                final float firstControlPointX = 0.5f * (prev.getX() + cur.getX());
                final float firstControlPointY = prev.getY();
                final float secondControlPointX = 0.5f * (prev.getX() + cur.getX());
                final float secondControlPointY = cur.getY();
                cubicPath.cubicTo(firstControlPointX, firstControlPointY,
     ...
     * @param bounds
     */
    protected void drawLinearFill(Canvas c, ILineDataSet dataSet, Transformer
            trans, XBounds bounds) {
